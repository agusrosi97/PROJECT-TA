<?php 
  ...
  $data_transaksi = query("SELECT tbl_transaksi_pembayaran.*, tbl_tamu.*, tbl_reservasi.*
    ...
    WHERE jam_transaksi BETWEEN DATE('".$awal_trans."') AND DATE('".$akhir_trans."') + INTERVAL 24 HOUR AND `status` IS NOT NULL
    ORDER BY id_transaksi DESC
  ");
  // Trans valid
  $transValid = mysqli_query($conn, "SELECT sum(total_pembayaran_kamar) AS total_bayar FROM tbl_transaksi_pembayaran
    WHERE jam_transaksi BETWEEN DATE('".$awal_trans."') AND DATE('".$akhir_trans."') + INTERVAL 24 HOUR AND `status` = 'VALID'");
  $outputValid = mysqli_fetch_assoc($transValid);
  // Trans gak valid
  $transGKValid = mysqli_query($conn, "SELECT sum(total_pembayaran_kamar) AS total_bayar FROM tbl_transaksi_pembayaran
    WHERE jam_transaksi BETWEEN DATE('".$awal_trans."') AND DATE('".$akhir_trans."') + INTERVAL 24 HOUR AND `status` = 'GAK VALID'");
  $outputGKValid = mysqli_fetch_assoc($transGKValid);
?>
  ...
  <tbody>
    ...
    ?>
      <tr>
        ...
        <td class="py-1"><a href="print_transaksiindv.php?id=<?=$row['id_transaksi']; ?>" class='btn btn-secondary px-2 py-1 rounded' target="_blank"><span><i class='fas fa-print'></i></span></a></td>
...
<div class="d-flex flex-wrap px-0">
  <h6 class="text-muted py-2 text-nowrap font-weight-bold mr-2">
    <span class="border border-success d-flex align-items-center pr-1 rounded">
      ...
      </span> Rp.<?=number_format($outputValid['total_bayar'],2,',','.'); ?>,-
    </span>
  </h6>
  <h6 class="text-muted py-2 text-nowrap font-weight-bold">
    <span class="border border-danger d-flex align-items-center pr-1 rounded">
      ...
      </span> Rp.<?=number_format($outputGKValid['total_bayar'],2,',','.'); ?>,-
    </span>
  </h6>
</div>
